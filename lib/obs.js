...
async function connect(password) {
  ...
  try {
    await obs.connect('ws://localhost:4455', password || undefined);
    connected = true;
    ...
    if (msg.includes('ECONNREFUSED')) {
      throw new Error(
        'Could not reach OBS WebSocket server on port 4455. ' +
        'In OBS, go to Tools → WebSocket Server Settings and make sure ' +
        '"Enable WebSocket server" is checked, then click OK and try again.'
      );
    }
    throw new Error(`OBS WebSocket connection failed: ${msg}`);
  }
}
